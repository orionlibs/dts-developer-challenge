...
export default function (app: Application): void {
  app.get('/', async (req, res) => {
    try {
      const response = await axios.get('http://localhost:4000/api/v1/cases');
  ...
  app.post('/cases', async (req, res) => {
    ...
    try {
      await axios.post(
        'http://localhost:4000/api/v1/cases',
        payload,
  ...
  app.delete('/cases/:id', async (req, res) => {
    ...
    try {
      await axios.delete(`http://localhost:4000/api/v1/cases/${caseId}`);
...
app.patch('/cases/statuses', async (req, res) => {
    ...
    try {
      await axios.patch(
        `http://localhost:4000/api/v1/cases/${caseIDToChange}/statuses`,
        payload,
        { headers: { 'Content-Type': 'application/json' } }
      );
